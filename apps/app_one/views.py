...
def login(request):
    ...
    if response_from_models['status']:#if true (validations are met):
        #saves user data in session, sends user to app2:
...
# 1st app handles only logging in / registering users
            ...
            # returns to index.html via named route app1, index method in views.py
            return redirect('app1:index')

    # if not POST, redirects to index method via named route namespace=app1
    else:
        return redirect('app1:index')
